...
<body>
	...
	<%
	    ...
	    <%
					...
					{
		%>
						<form name=g method=post action=addresses.jsp>
					<tr>
						
						<th><input type=text name=t1 readonly value=<%=rs.getString(1)%> onkeypress="msg()"></th>
		  				...
		  				<th><input type=submit value="Update" name=b1></th>
	    ...
	    <%		}
			...
			Statement smt = conn.createStatement();
			ResultSet rs;
			if(colnm.equalsIgnoreCase("All_Details"))
					      rs=smt.executeQuery("select * from addresses");
					else
			              rs = smt.executeQuery("select * from addresses where"+" "+colnm + "="+ "'"+t1+"'");
			%>
			<table border=2>
				<tr>
					<th>Addres ID</th>
